...
import { HiAcademicCap, HiAdjustmentsVertical, HiCalendar, HiChartBar, HiMiniSquares2X2, HiUser, HiUserGroup } from "react-icons/hi2";
import { toast } from "react-toastify";
import { IoLogOut } from "react-icons/io5";
import { Avatar, Button } from "antd";

const links = [
  { href: '/admin', label: 'Dashboard', icon: HiMiniSquares2X2 },
  { href: '/admin/merchants', label: 'Merchants', icon: HiUserGroup },
  { href: '/admin/market-index', label: 'Market Index', icon: HiChartBar },
...
export default function DashboardLayout({ children }) {
  ...
  return (
    <section className="w-screen h-screen overflow-hidden flex flex-row">
      <div className="flex flex-col text-gray-700 bg-gray-700 text-white gap-y-4 pb-4 border-l w-[250px] max-sm:w-[70px]">
        ...
        <div className="flex-grow">
          {links.map((link) => (
            <Link key={link.href} href={link.href} className={`p-4 w-full flex flex-row items-center justify-start duration-150 ${pathname === link.href ? 'bg-gray-900' : 'hover:bg-gray-800'}`}>
      ...
      <div className="w-full">
        <div className="h-full border-l bg-gray-100 overflow-y-auto">
          <div className="p-4 text-gray-700 bg-white w-full flex flex-row items-center justify-between mb-4 border-b top-0 sticky">
          ...
          <div className="p-4 flex flex-col gap-y-4">
            ...
            <div className="flex text-xs flex-row gap-x-2">
              ...
              <p>/</p>
              <Link href={'/merchant'} className="text-gray-900">Merchant</Link>
              {pathname !== '/merchant' && (
                <>
                  <p>/</p>
                  <Link href={pathname} className="text-gray-900">{pathname.replace('/merchant/', '')}</Link>
                </>
              )}
            </div>
            {children}
